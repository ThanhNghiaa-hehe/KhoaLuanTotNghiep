...
if "videberta" in MODEL_NAME:
    EPOCHS                      = 10
    LEARNING_RATE               = 3e-5  # Tối ưu hóa cho videberta-xsmall (xsmall học tốt ở tốc độ 3e-5)
    WARMUP_RATIO                = 0.1
    ADAM_EPSILON                = 1e-6
    EARLY_STOPPING_PATIENCE     = 3
    TRAIN_BATCH_SIZE            = 8  # xsmall rất nhẹ, batch size 8 cực kỳ an toàn trên GPU 4GB VRAM
    GRADIENT_ACCUMULATION_STEPS = 4  # Tích lũy 4 bước để có kích thước lô hiệu dụng = 32
else:
    EPOCHS                      = 5
    LEARNING_RATE               = 2e-5
    WARMUP_RATIO                = 0.0
...
if "videberta" in MODEL_NAME:
    ...
    # Vá lỗi HuggingFace: Khởi tạo trọng số thủ công cho các mô hình DeBERTa/ViDeBERTa ở định dạng float16
    print("\n[transformers] Vá lỗi HuggingFace: Đang khởi tạo thủ công lớp Classifier và LayerNorm...")
    torch.nn.init.normal_(model.classifier.weight, std=0.02)
    torch.nn.init.zeros_(model.classifier.bias)
    torch.nn.init.ones_(model.layernorm.weight)
    torch.nn.init.zeros_(model.layernorm.bias)
    print("[transformers] Hoàn tất khởi tạo thủ công thành công!\n")
else:
...
training_args = TrainingArguments(
    ...
    fp16                        = False,   # Chuyển từ True thành False để dùng FP32 chuẩn
