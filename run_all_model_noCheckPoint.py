...
if USE_AUGMENTATION:
    try:
        ...
        pass  # Xử lý cho môi trường Colab khi không có file vật lý
    
    # Đưa hàm gọi ra ngoài try-except để đảm bảo luôn được chạy
...
def run_experiment(model_name):
    ...
    # 1. Tokenizer
    use_word_seg = "phobert" in model_name.lower() or "vibert" in model_name.lower()
    ...
    TrainerClass = Trainer if "videberta" in model_name.lower() else CustomLossTrainer
    ...
    # =============================================================================
    # XÓA CHECKPOINT ĐỂ TIẾT KIỆM GOOGLE DRIVE
    # =============================================================================
    print("\n" + "="*70)
    print("BLOCK 6 — XÓA CHECKPOINT (TIẾT KIỆM BỘ NHỚ)")
    print("="*70)
    print(f"🧹 Đang dọn dẹp các thư mục Checkpoint khổng lồ...")
    checkpoint_dirs = glob.glob(os.path.join(OUTPUT_DIR, "checkpoint-*"))
    for c_dir in checkpoint_dirs:
        try:
            shutil.rmtree(c_dir)
            print(f"   Đã xóa thành công: {os.path.basename(c_dir)}")
        except Exception as e:
            print(f"   Lỗi khi xóa {c_dir}: {e}")
    if not checkpoint_dirs:
        print("   Không tìm thấy Checkpoint nào cần xóa.")
    print("✅ Đã dọn dẹp xong Checkpoint!")
    ...
    # DỌN DẸP BỘ NHỚ GPU - BƯỚC QUAN TRỌNG NHẤT
